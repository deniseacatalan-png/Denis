...
test("homepage exposes local real estate SEO metadata", async () => {
  ...
  assert.match(html, /<link rel="canonical" href="https:\/\/denisecatalanbienesraices\.com\/" \/>/);
  assert.match(html, /<meta property="og:url" content="https:\/\/denisecatalanbienesraices\.com\/" \/>/);
  ...
  assert.equal(jsonLd.url, "https://denisecatalanbienesraices.com/");
...
test("robots and sitemap expose the public site to search engines", async () => {
  ...
  assert.match(robots, /Sitemap: https:\/\/denisecatalanbienesraices\.com\/sitemap\.xml/);

  assert.match(sitemap, /<loc>https:\/\/denisecatalanbienesraices\.com\/<\/loc>/);
